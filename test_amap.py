...
"""
from agent.tools.poi import search_poi
...
def main():
    ...
    # 测试 1: 正常搜索
    print("\n>>> 测试 1：搜索「北京 咖啡厅」")
    result = search_poi(keywords="咖啡厅", location="北京", limit=3)
    pois = __import__("json").loads(result)
    if isinstance(pois, list):
        ...
        for i, poi in enumerate(pois, 1):
            ...
            print()
    elif isinstance(pois, dict) and "error" in pois:
        print(f"❌ 错误: {pois['error']}")

    # 测试 2: 空结果
    print(">>> 测试 2：搜索不存在的关键词")
    result = search_poi(keywords="zzz_not_exist_zzz", location="北京", limit=3)
