...
def filter_by_category(pois: list) -> list:
    ...
    return [
        p
        for p in pois
        if not any(b in (p.get("category", "") + p.get("type", "") + p.get("name", "")) for b in CATEGORY_BLACKLIST)
    ]
...
def filter_near_anchor(pois: list, anchor_coords: tuple, anchor_name: str = "", min_distance: int = 100) -> list:
