...
class ModificationIntent(BaseModel):
    ...
    change_type: str = "none"          # change_origin / change_destination / change_keywords /
                                       ...
                                       # add_poi / adjust_constraint / new_route / none
...
# 加入/添加 POI（优先于 destination/keywords/num_stops 规则）
_ADD_POI_PATTERNS = [
    (r"(?:多去一个|多去|再加一个|再加|多加一个|多加|加入|添加|加上|增加|插入|顺路去|顺便去|顺便看看)\s*[「「]?(.{2,20}?)[」」]?\s*(?:吧|吧！|呢|！|$)", "add_poi"),
]
...
_ALL_RULES = (
    _ORIGIN_PATTERNS + _ADD_POI_PATTERNS + _DEST_PATTERNS + _KEYWORD_PATTERNS +
    ...
    for pattern, change_type in _ALL_RULES:
        ...
        if m:
            ...
            if change_type == "change_origin":
                ...
                # 清理捕获：去掉"吧""呢""个""一个"等干扰词
                name = re.sub(r"[吧呢！!。.]$", "", captured).strip()
                name = re.sub(r"^[一个]+", "", name)
                name = re.sub(r"[吧呢！!。.]$", "", name).strip()  # 双重保证
                params["poi_name"] = name if name else captured
...
- add_poi: 在现有路线中加入一个具体的地点（如"加入大兴善寺"、"顺便去钟楼"）
...
关键区分：
- "加入/添加/加上/顺便去 + 具体地名" → add_poi，params={"poi_name": "地名"}
- "去/换到/终点改成 + 地名" → change_destination
- add_poi 是在保留原有起终点的情况下中途新增一个途经点
