...
class RouteConstraints:
    ...
    max_duration_min: Optional[int] = None  # 180
    
    # Soft preferences (尽量满足)
    preferred_categories: List[str] = field(default_factory=list)  # ["公园","咖啡"]
    vibe: Optional[str] = None  # "安静"|"热闹"|"浪漫"|"亲子"
    
    _source_rounds: dict = field(default_factory=dict)
    
    def is_empty(self) -> bool:
        return not any([
            self.budget, self.dietary, self.exclusions, self.must_include,
            self.max_duration_min, self.preferred_categories, self.vibe
        ])
    ...
    def merge(self, user_input: str, round_num: int = 1) -> 'RouteConstraints':
        ...
        if any(w in user_input for w in ["素食","不吃肉","纯素","吃素"]):
            if "素食" not in new.dietary: new.dietary.append("素食")
        if any(w in user_input for w in ["不辣","无辣","不吃辣","不要辣","清淡"]):
            if "无辣" not in new.dietary: new.dietary.append("无辣")
        if any(w in user_input for w in ["清真","回民"]):
            if "清真" not in new.dietary: new.dietary.append("清真")
        ...
        if tm:
            ...
            if tm:
                dur = int(tm.group(1))
                if dur < 720: new.max_duration_min = dur
        
        # Must-include places
        known = ["大雁塔","回民街","钟楼","鼓楼","大唐不夜城","兵马俑","小雁塔",
                 "城墙","曲江","浐灞","小寨","北站","高新","秦岭"]
        for place in known:
    ...
    def from_dict(cls, d: dict) -> 'RouteConstraints':
        if not d: return cls()
        return cls(
            budget=d.get("budget"), dietary=d.get("dietary",[]),
            exclusions=d.get("exclusions",[]), must_include=d.get("must_include",[]),
            max_duration_min=d.get("max_duration_min"),
            preferred_categories=d.get("preferred_categories",[]), vibe=d.get("vibe"),
        )
