...
for i, sc in enumerate(ROUND_1):
    ...
    for j, query in enumerate(sc["rounds"]):
        ...
        try:
            ...
            results.append({"round": j+1, "query": query,
                "narration_preview": narration[:200].replace("\n", " "),
                "stops": stops, "num_stops": len(stops),
                "elapsed_s": elapsed, "score": round(score, 1), "issues": issues})
    ...
    r = {"id": uid, "desc": sc["desc"], "total_rounds": len(results),
         "total_time_s": round(total_time, 1), "avg_score": avg, "results": results}
...
with open(OUTPUT_DIR / "test_round1_report.json", "w") as f:
    json.dump({"round": 1, "overall_avg": overall, "ok": ok, "warn": warn, "fail": fail, "results": all_results}, f, ensure_ascii=False, indent=2)
