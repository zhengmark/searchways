...
| 模块 | 文件 | 状态 | 说明 |
|------|------|------|------|
| LLM 意图解析 | `core.py:_parse_intent` | ✅ 完成 | prompt 已加固（防截断/占位符校验/数量求和） |
| 地理编码 | `poi.py:robust_geocode` | ✅ 完成 | 4 层递进兜底 + 正则回退 + 子串校验 |
| POI 搜索 | `poi.py:search_poi/around/along_route` | ✅ 完成 | 高德 text/around 双 API + 走廊 4 点采样 |
| 图构建 | `graph_planner.py:build_graph` | ✅ 完成 | 全连接邻接矩阵，ThreadPool 并发，步行 API + haversine 兜底 |
| 路径规划 | `graph_planner.py:shortest_path` | ✅ 完成 | 连线投影分段选取（有终点）/ 距离带分层（无终点） |
| 路线解说 | LLM 生成 | ✅ 完成 | 算法结果 → LLM 通俗化，try/except 降级保护 |
...
### 核心数据流（`run_agent` 6 步流水线）

```
用户输入 "从西安丈八六路地铁站出发到浐灞玩"
  │
  ├─ 1. _extract_city()     → 城市名（正则匹配 300+ 城市列表 + "XX市" 兜底）
  ├─ 2. _parse_intent()     → LLM 提取 {origin, destination, keywords, num_stops} JSON
  ...
  ├─ 4. _search_corridor_pois() → 沿途 POI 搜索
  │      └─ 关键词规范化 (吃→美食, 宵夜→小吃烧烤)
  │      └─ search_along_route(起,终) + search_around(终点) 或 search_around(起点,双半径)
  │      └─ 品类黑名单过滤 (打印店/维修/药店/中介)
  ├─ 5. build_graph() + shortest_path()
  │      └─ 全连接邻接矩阵 → 连线投影分段选取 → 200m 最小间距过滤
  ├─ 6. LLM 解说 + Mermaid + Leaflet HTML → 输出
  │      └─ try/except 保护，崩溃时输出核心数据 + Mermaid
  └─ 产出 3 个文件：文本回复 + route_output.md + route_output.html
```
...
| P1 | 多轮对话 | `AgentSession.messages` 已存历史，但 `run_agent` 的 `is_new` 分支只支持单轮；需打通第二轮走 else 分支的调整逻辑 |
...
*生成时间：2026-04-27 | 代码总计 2070 行 Python*
